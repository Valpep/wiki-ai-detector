# AI Vocabulary Word Lists by Model Era

Source: Wikipedia:Signs of AI writing (empirically documented overuse in LLM output)

## GPT-4 era (2023 – mid-2024)

- Additionally
- boasts
- bolstered
- crucial
- delve
- emphasizing
- enduring
- garner
- interplay
- intricate
- intricacies
- key (as adjective)
- landscape (abstract use)
- meticulous
- meticulously
- pivotal
- tapestry (abstract use)
- testament
- underscore (as verb)
- valuable
- vibrant

**Example density test:** "This *pivotal* initiative *underscores* the *enduring* *testament* to the region's *vibrant* culture, offering *valuable* *insights* into the *intricate* *tapestry* of its history."

---

## GPT-4o era (mid-2024 – mid-2025)

- align with
- bolstered
- crucial
- emphasizing
- enhance
- enduring
- fostering
- highlighting
- pivotal
- showcasing
- underscore
- vibrant

---

## GPT-5 / newer era (mid-2025+)

- emphasizing
- enhance
- highlighting
- showcasing

**Notability/media language:**
- independent coverage
- profiled in
- active social media presence
- local/regional/national media outlets
- written about in

---

## Significance inflation trigger phrases

- stands as
- serves as
- marks a pivotal
- marks a significant
- represents a shift
- underscores its importance
- reflects broader
- symbolizing its enduring
- setting the stage for
- indelible mark
- deeply rooted
- evolving landscape
- focal point
- key turning point
- contributing to the broader
- shaping the
- in the heart of

---

## Promotional / travel-guide words

- nestled
- vibrant
- rich (cultural heritage)
- breathtaking
- diverse array
- boasts
- showcasing
- groundbreaking
- renowned
- in the heart of
- captivates
- dynamic hub
- rich tapestry
- commitment to excellence
- dedication to

---

## Superficial analysis -ing phrases (attached to end of sentences)

- highlighting its importance
- underscoring its significance
- reflecting the broader trend
- emphasizing the need for
- contributing to the understanding of
- fostering a deeper appreciation
- cultivating a sense of
- encompassing a wide range
- ensuring continued relevance
- symbolizing the ongoing effort

---

## Vague attribution weasel phrases

- Experts argue
- Observers have noted
- Observers have cited
- Industry reports suggest
- Several sources indicate
- Some critics argue
- According to analysts
- Researchers suggest
- Studies have shown (without citation)

---

## Negative parallelism patterns

- Not only X, but also Y
- It's not just X, it's Y
- Not merely X, but Y
- This is not X — it is Y
- No X, no Y — just Z
- Rather than X, it Y
- While X, it also Y

---

## Communication leakage phrases

- I hope this helps
- Would you like me to
- Let me know if you need
- Is there anything else
- Here is a detailed overview
- Below is a comprehensive
- I've outlined the key points
- Feel free to ask
- Of course!
- Certainly!

---

## Knowledge-cutoff disclaimer patterns

- as of my last knowledge update
- as of my last training update
- up to my last update
- not widely documented
- not extensively covered
- specific details are limited
- while information is scarce
- likely supports (speculative ecology)
- maintains a low profile
- keeps personal details private
- chooses not to share
